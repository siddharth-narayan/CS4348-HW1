...
void color_clear() { printf("\e[0m"); }

void color_bold() { printf("\e[1m"); }
...
void print_array(int *ptr, int len, char *ansi_color_format) {

    if (ansi_color_format != NULL) {
        printf("%s", ansi_color_format);
    }
...
void print_bytes(char *buf, int len, char *ansi_color_format) {
    if (ansi_color_format != NULL) {
        printf("%s", ansi_color_format);
    }
...
inline void debug(uint32_t mode, char *format, ...) {
    if (mode > DEBUG_MODE) {
        return;
    }
